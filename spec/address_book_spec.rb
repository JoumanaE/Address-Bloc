...
# #3
  ...
  describe "#iterative_search" do
    it "iterates over the entries until it finds the match" do
      book.import_from_csv("entries.csv")
      entry = book.iterative_search("Bill")
      expect(entry).to be_a Entry
      expect(book.iterative_search('Bill')).to eq("Bill")
    end
    it "iterates over the entries until it finds the match" do
      book.import_from_csv("entries.csv")
      entry = book.iterative_search("Sussie")
      expect(book.iterative_search('Sussie')).to eq("Sussie")
    end
    it "returns nil if there is no entry found" do
      book.import_from_csv("entries.csv")
      expect(address_book.iterative_search("Apple Pie")).to be_nil
    end
  end
end
